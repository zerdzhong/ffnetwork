...
RequestImpl::RequestImpl(
    ...
    : url_(url), headers_(header_map), method_(GetMethod), data_(nullptr),
      data_length_(0) {}

RequestImpl::RequestImpl(const Request &request)
    : url_(request.url()), headers_(request.headerMap()),
      method_(request.method()), data_(nullptr), data_length_(0) {
...
std::string RequestImpl::url() const { return url_; }

void RequestImpl::setUrl(const std::string &url) { url_ = url; }
...
std::string RequestImpl::method() const { return method_; }

void RequestImpl::setMethod(const std::string &method) { method_ = method; };
...
std::string RequestImpl::serialise() const { return ""; }
} // namespace ffnetwork

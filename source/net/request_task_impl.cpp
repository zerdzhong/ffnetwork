...
namespace ffnetwork {
    ...
    RequestTaskImpl::RequestTaskImpl(const std::shared_ptr<Request> request,
									 const std::weak_ptr<RequestTaskDelegate> &delegate) :
			request_(request),
			cancelled_(false),
            delegate_(delegate),
            metrics_(std::make_shared<Metrics>()),
			identifier_(request->hash())
    {
...
CURL *RequestTaskImpl::handle() {
	return handle_->handle;
}

void RequestTaskImpl::setCompletionCallback(CompletionCallback completionCallback) {
...
size_t RequestTaskImpl::write_callback(char *data, size_t size, size_t nitems, void *str) {
...
size_t RequestTaskImpl::header_callback(char *data, size_t size, size_t nitems, void *str) {
...
void RequestTaskImpl::didFinished(HttpStatusCode http_code, ResponseCode response_code) {
...
void RequestTaskImpl::didCancel() {
	std::shared_ptr<Response> cancelled_response = std::make_shared<ResponseImpl>(
			request_, nullptr, 0, HttpStatusCode::StatusCodeInvalid, ResponseCode::UserCancel, metrics_, true);

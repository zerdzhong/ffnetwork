...
#pragma mark- public_method
    ...
    std::shared_ptr<RequestTask> CurlClient::PerformRequest(const std::shared_ptr<Request> &request,
	...
	std::shared_ptr<RequestTask> CurlClient::TaskWithRequest(const std::shared_ptr<Request> &request,
			CompletionCallback callback) {

    	std::unique_lock<std::mutex> client_lock(client_mutex_);
		std::string request_hash = request->hash();

		auto request_task = std::make_shared<RequestTaskImpl>(request, shared_from_this());
...
#pragma mark- RequestTaskDelegate
	...
	void CurlClient::RequestTaskStart(const std::shared_ptr<RequestTask> &task) {
		std::string request_hash = task->taskIdentifier();
		request_thread_.GetTaskRunner()->PostTask(std::bind(&CurlClient::StartRequest, this, request_hash));
	}

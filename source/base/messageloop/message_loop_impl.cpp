...
MessageLoopImpl::MessageLoopImpl()
    : task_queues_(MessageLoopTaskQueues::GetInstance()),
      queue_id_(task_queues_->CreateTaskQueue()), terminated_(false) {
  task_queues_->SetWakeable(queue_id_, this);
}

MessageLoopImpl::~MessageLoopImpl() { task_queues_->Dispose(queue_id_); }

#pragma mark - task_method
void MessageLoopImpl::PostTask(std::function<void()> task,

...
TaskQueueId MessageLoopTaskQueues::CreateTaskQueue() {
    ...
    queue_entries_[loop_id] = std::unique_ptr<TaskQueueEntry>(new TaskQueueEntry());
...
void MessageLoopTaskQueues::RegisterTask(TaskQueueId queue_id, std::function<void()> task, TimePoint target_time) {
    ...
    queue_entry->delayed_tasks.push({static_cast<size_t>(order), std::move(task), target_time});
...
void MessageLoopTaskQueues::AddTaskObserver(TaskQueueId queue_id, intptr_t key, std::function<void()> callback) {
    ScopedMutex queue_lock(GetMutex(queue_id));
...
void MessageLoopTaskQueues::RemoveTaskObserver(TaskQueueId queue_id, intptr_t key) {
    ScopedMutex queue_lock(GetMutex(queue_id));
    
    queue_entries_[queue_id]->task_observers.erase(key);
}

std::vector<closure> MessageLoopTaskQueues::GetObserversToNotify(TaskQueueId queue_id) const {
    ScopedMutex queue_lock(GetMutex(queue_id));
...
void MessageLoopTaskQueues::SetWakeable(TaskQueueId queue_id, Wakeable* wakeable) {
    ScopedMutex queue_lock(GetMutex(queue_id));
...
void MessageLoopTaskQueues::WakeUpUnlocked(TaskQueueId queue_id, TimePoint time) const {
...
const DelayedTask& MessageLoopTaskQueues::PeekNextTaskUnlocked(TaskQueueId owner_id,

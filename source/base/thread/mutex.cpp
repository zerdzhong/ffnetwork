...
public:
  SharedMutexPosix() { pthread_rwlock_init(&rw_lock_, nullptr); }

  ~SharedMutexPosix() { pthread_rwlock_destroy(&rw_lock_); }
...
void SharedMutexPosix::Lock() { pthread_rwlock_wrlock(&rw_lock_); }

void SharedMutexPosix::LockShared() { pthread_rwlock_rdlock(&rw_lock_); }

void SharedMutexPosix::Unlock() { pthread_rwlock_unlock(&rw_lock_); }

void SharedMutexPosix::UnlockShared() { pthread_rwlock_unlock(&rw_lock_); }

bool SharedMutexPosix::TryLock() { return pthread_rwlock_trywrlock(&rw_lock_); }

...
TEST(MessageLoopTaskQueues, RegisterTask) {
    ...
    task_queues->SetWakeable(queue_id, new TestWakeable([&time](TimePoint wake_time){
        ASSERT_TRUE(time == wake_time);
    }));
    
    task_queues->RegisterTask(queue_id, []{}, time);
...
TEST(MessageLoopTaskQueues, RegisterTwoTasksAndCount) {
    ...
    task_queues->RegisterTask(queue_id, []{}, TimePoint::Now());
    task_queues->RegisterTask(queue_id, []{}, TimePoint::Max());
...
TEST(MessageLoopTaskQueues, WakeUpIndependentOfTime) {
    ...
    task_queues->SetWakeable(queue_id, new TestWakeable([&num_wakes](TimePoint wake_time){
        ++num_wakes;
    }));
